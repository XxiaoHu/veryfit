...
class Video_Content_Manager {
    ...
    private function __construct() {
        ...
        add_action( 'wp_ajax_sync_video_test_to_video_content', array( $this, 'ajax_sync_video_test_to_video_content' ) );
        add_action( 'save_post_video_test', array( $this, 'auto_sync_video_test_on_save' ), 10, 3 );
        add_action( 'before_delete_post', array( $this, 'auto_sync_video_test_on_delete' ), 10, 2 );
    }
    ...
    public function video_content_template_include( $template ) {
        ...
        if ( defined('WP_DEBUG') && WP_DEBUG ) {
            error_log('video_content_archive query var: ' . get_query_var( 'video_content_archive' ));
            error_log('Current template: ' . $template);
            error_log('is_post_type_archive(video_content): ' . (is_post_type_archive('video_content') ? 'yes' : 'no'));
        }
        ...
        if ( get_query_var( 'video_content_archive' ) ) {
            ...
            if ( file_exists( $custom_template ) ) {
                if ( defined('WP_DEBUG') && WP_DEBUG ) {
                    error_log('Loading custom template: ' . $custom_template);
                }
    ...
    public function render_sync_page() {
        ?>
        <div class="wrap">
            <h1><?php _e( 'Video Test 数据同步到 Video Content', 'feryfit' ); ?></h1>

            <div class="card">
                <h2><?php _e( '同步说明', 'feryfit' ); ?></h2>
                <p><?php _e( '此功能将把所有 Video Test 文章类型的数据同步到 Video Content 文章类型。', 'feryfit' ); ?></p>
                <ul>
                    ...
                    <li><?php _e( '⚠ 已存在的 Video Content 数据不会被删除', 'feryfit' ); ?></li>
                </ul>
            </div>
            ...
            <div class="card">
                ...
                <p>
                    <strong><?php _e( 'Video Test 总数：', 'feryfit' ); ?></strong>
                    <span id="video-test-count">
                        <?php echo wp_count_posts( 'video_test' )->publish; ?>
                    </span>
                </p>
                <p>
        ...
        jQuery(document).ready(function($) {
            $('#sync-video-btn').on('click', function() {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data: {
                        action: 'sync_video_test_to_video_content',
                        nonce: '<?php echo wp_create_nonce( 'video_content_sync_nonce' ); ?>'
                    },
                    success: function(response) {
                        $btn.prop('disabled', false).text('<?php _e( '开始同步', 'feryfit' ); ?>');
                        $progress.hide();
    ...
    /**
     * AJAX sync video test to video content
     */
    public function ajax_sync_video_test_to_video_content() {
        ...
        $result = $this->sync_video_test_to_video_content();
    ...
    /**
     * Sync video test to video content
     */
    public function sync_video_test_to_video_content() {
        $video_tests = get_posts( array(
            'post_type'      => 'video_test',
            'posts_per_page' => -1,
            'post_status'    => 'any',
            'orderby'        => 'ID',
            'order'          => 'ASC',
        ...
        if ( empty( $video_tests ) ) {
            return array(
                'success' => false,
                'message' => __( '没有找到需要同步的 Video Test 数据', 'feryfit' ),
            );
        }
        ...
        foreach ( $video_tests as $video_test ) {
            $result = $this->sync_single_video_test( $video_test->ID );
            if ( $result ) {
                $synced_count++;
            } else {
                $error_count++;
        ...
        $message = sprintf(
            __( '同步完成！成功同步 %d 条记录，失败 %d 条。', 'feryfit' ),
            $synced_count,
            $error_count
        );
    ...
    /**
     * Sync single video test to video content
     */
    public function sync_single_video_test( $video_test_id ) {
        $video_test = get_post( $video_test_id );
        if ( ! $video_test || $video_test->post_type !== 'video_test' ) {
            return false;
        }
        ...
        $existing = get_posts( array(
            ...
            'meta_key'   => '_synced_from_video_test_id',
            'meta_value' => $video_test_id,
            'fields'     => 'ids',
        ) );
        ...
        if ( ! empty( $existing ) ) {
            ...
            $updated = wp_update_post( array(
                'ID'           => $video_content_id,
                'post_title'   => $video_test->post_title,
                'post_content' => $video_test->post_content,
                'post_status'  => $video_test->post_status,
                'post_date'    => $video_test->post_date,
                'menu_order'   => $video_test->menu_order,
            ) );
            ...
            update_post_meta( $video_content_id, '_synced_from_video_test_id', $video_test_id );
        }

        // Sync thumbnail
        $thumbnail_id = get_post_thumbnail_id( $video_test_id );
        if ( $thumbnail_id ) {
            set_post_thumbnail( $video_content_id, $thumbnail_id );
        }

        // Sync all meta data
        $meta_data = get_post_meta( $video_test_id );
    ...
    /**
     * Auto sync video test on save
     */
    public function auto_sync_video_test_on_save( $post_id, $post, $update ) {
        ...
        $this->sync_single_video_test( $post_id );
    }

    /**
     * Auto sync video test on delete
     */
    public function auto_sync_video_test_on_delete( $post_id, $post ) {
        if ( $post->post_type !== 'video_test' ) {
            return;
        }
