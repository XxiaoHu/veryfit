<?php
/**
 * FAQ Pagination Block Renderer
 *
 * @package feryfit
 */

$title = isset( $attributes['title'] ) ? $attributes['title'] : 'FAQ';
$posts_per_page = isset( $attributes['postsPerPage'] ) ? $attributes['postsPerPage'] : 10;

$wrapper_attributes = get_block_wrapper_attributes();

if ( ! function_exists( 'feryfit_faq_pagination_limit_title' ) ) {
	function feryfit_faq_pagination_limit_title( $title, $length = 80 ) {
...
if ( ! function_exists( 'feryfit_faq_pagination_get_page_url' ) ) {
	function feryfit_faq_pagination_get_page_url( $page_num ) {
		if ( $page_num === 1 ) {
			return get_permalink();
		}
		return add_query_arg( 'paged', $page_num, get_permalink() );
	}
}
...
// 手动排序：置顶优先（使用 _faq_is_pinned meta 字段），然后按修改时间降序
$sticky_faqs = array();
$normal_faqs = array();
...
if ( $current_page > $total_pages ) {
	wp_redirect( feryfit_faq_pagination_get_page_url( $total_pages ) );
	exit;
}
...
?>
<div <?php echo $wrapper_attributes; ?> style="background: #fff;">
	<div class="faq-pagination">
		<div class="faq-pagination__header">
			<h2 class="faq-pagination__title"><?php echo esc_html( $title ); ?></h2>
		</div>

		<ul class="faq-pagination__items">
			...
			<?php foreach ( array_slice( $posts, ( $current_page - 1 ) * $posts_per_page, $posts_per_page ) as $post ) : ?>
				<li class="faq-pagination__item">
					<a href="<?php echo esc_url( home_url( '/archives/faq/' . $post->ID . '/' ) ); ?>">
						...
						<span class="faq-pagination__item-title">
							<?php echo esc_html( $display_index ); ?>.
							<?php echo esc_html( feryfit_faq_pagination_limit_title( $post->post_title, 80 ) ); ?>
						</span>
						<img src="<?php echo esc_url( $arrow_icon ); ?>" alt="" class="faq-pagination__item-arrow" />
					</a>
				</li>
				<?php $display_index++; ?>
			<?php endforeach; ?>
		...
		<div class="faq-pagination__nav-wrapper">
			<?php if ( $total_pages > 1 ) : ?>
				<div class="faq-pagination__nav-container">
					<?php if ( $current_page == 1 ) : ?>
						<span class="faq-pagination__nav faq-pagination__nav--prev faq-pagination__nav--disabled">
							<svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
								<g clip-path="url(#clip0_187_1642_prev)">
								...
								<defs>
									<clipPath id="clip0_187_1642_prev">
					...
					<?php else : ?>
						<a href="<?php echo esc_url( feryfit_faq_pagination_get_page_url( $current_page - 1 ) ); ?>" 
							class="faq-pagination__nav faq-pagination__nav--prev">
							<svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
								<g clip-path="url(#clip0_187_1642_prev_active)">
								...
								<defs>
									<clipPath id="clip0_187_1642_prev_active">
										<rect width="30" height="30" fill="white"/>
									</clipPath>
								</defs>
							</svg>
					...
					<div class="faq-pagination__pages">
						<?php
						$max_visible = 5;
						...
						if ( $start_page > 1 ) {
							?>
							<a href="<?php echo esc_url( feryfit_faq_pagination_get_page_url( 1 ) ); ?>" 
								class="faq-pagination__page">1</a>
							<?php
							if ( $start_page > 2 ) {
								echo '<span class="faq-pagination__dots">...</span>';
							}
						}

						for ( $i = $start_page; $i <= $end_page; $i++ ) {
							?>
							<a href="<?php echo esc_url( feryfit_faq_pagination_get_page_url( $i ) ); ?>" 
								class="faq-pagination__page <?php echo $current_page === $i ? 'faq-pagination__page--active' : ''; ?>">
								<?php echo esc_html( $i ); ?>
							</a>
							<?php
						}
						...
						if ( $end_page < $total_pages ) {
							...
							?>
							<a href="<?php echo esc_url( feryfit_faq_pagination_get_page_url( $total_pages ) ); ?>" 
								class="faq-pagination__page"><?php echo esc_html( $total_pages ); ?></a>
							<?php
						}
						?>
					</div>

					<div class="faq-pagination__mobile-counter">
						<span class="faq-pagination__current"><?php echo esc_html( $current_page ); ?></span>
						<span class="faq-pagination__separator">/</span>
						<span class="faq-pagination__total"><?php echo esc_html( $total_pages ); ?></span>
					</div>

					<?php if ( $current_page == $total_pages ) : ?>
						<span class="faq-pagination__nav faq-pagination__nav--next faq-pagination__nav--disabled">
							<svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
								<g clip-path="url(#clip0_187_1644_next)">
								...
								<defs>
									<clipPath id="clip0_187_1644_next">
					...
					<?php else : ?>
						<a href="<?php echo esc_url( feryfit_faq_pagination_get_page_url( $current_page + 1 ) ); ?>" 
							class="faq-pagination__nav faq-pagination__nav--next">
							<svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
								<g clip-path="url(#clip0_187_1644_next_active)">
									<path d="M11.5875 7.85C11.685 7.75211 11.801 7.67444 11.9286 7.62145C12.0562 7.56845 12.1931 7.54117 12.3312 7.54117C12.4694 7.54117 12.6063 7.56845 12.7339 7.62145C12.8615 7.67444 12.9774 7.75211 13.075 7.85L20 14.275C20.1056 14.3733 20.1899 14.4922 20.2475 14.6245C20.3051 14.7568 20.3348 14.8995 20.3348 15.0438C20.3348 15.188 20.3051 15.3307 20.2475 15.463C20.1899 15.5953 20.1056 15.7142 20 15.8125L13.1 22.275C12.8961 22.4656 12.6248 22.5674 12.3459 22.5581C12.0669 22.5487 11.8031 22.4289 11.6125 22.225C11.4219 22.0211 11.32 21.7499 11.3294 21.4709C11.3388 21.1919 11.4586 20.9281 11.6625 20.7375L17.7125 15L11.625 9.3C11.4334 9.10954 11.3226 8.85257 11.3156 8.58252C11.3086 8.31247 11.406 8.05011 11.5875 7.85ZM15 30C17.9667 30 20.8668 29.1203 23.3335 27.472C25.8003 25.8238 27.7229 23.4811 28.8582 20.7403C29.9935 17.9994 30.2906 14.9834 29.7118 12.0736C29.133 9.16393 27.7044 6.49119 25.6066 4.3934C23.5088 2.29561 20.8361 0.867001 17.9263 0.288223C15.0166 -0.290556 12.0006 0.00649452 9.25974 1.14181C6.51885 2.27712 4.17617 4.19971 2.52795 6.66645C0.879728 9.13318 -5.72205e-06 12.0333 -5.72205e-06 15C-5.72205e-06 18.9782 1.58035 22.7936 4.39339 25.6066C7.20644 28.4196 11.0217 30 15 30Z" fill="black"/>
								</g>
								<defs>
									<clipPath id="clip0_187_1644_next_active">
										<rect width="30" height="30" fill="white" transform="matrix(-1 0 0 -1 30 30)"/>
									</clipPath>
								</defs>
							</svg>
