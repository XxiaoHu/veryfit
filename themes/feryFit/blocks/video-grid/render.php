...
$section_title = isset($attributes['sectionTitle']) ? $attributes['sectionTitle'] : 'Video';
$posts_per_page = 12;

$wrapper_attributes = get_block_wrapper_attributes();

if (! function_exists('feryfit_video_pagination_get_page_url')) {
	function feryfit_video_pagination_get_page_url($page_num) {
		if ($page_num === 1) {
...
if ($current_page > $total_pages) {
	wp_redirect(feryfit_video_pagination_get_page_url($total_pages));
	exit;
}
...
<div <?php echo $wrapper_attributes; ?> style="background-color: #fff;">
	<h2 class="video-grid__title"><?php echo esc_html($section_title); ?></h2>
	...
	<div class="video-grid__pagination">
		<?php if ($total_pages > 1) : ?>
			<div class="video-grid__pagination-container">
				<?php if ($current_page == 1) : ?>
					<span class="video-grid__pagination-nav video-grid__pagination-nav--prev video-grid__pagination-nav--disabled">
				...
				<?php else : ?>
					<a href="<?php echo esc_url(feryfit_video_pagination_get_page_url($current_page - 1)); ?>" 
						class="video-grid__pagination-nav video-grid__pagination-nav--prev">
						<svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
				...
				<div class="video-grid__pagination-pages">
					<?php
					$max_visible = 5;
					...
					if ($start_page > 1) {
						?>
						<a href="<?php echo esc_url(feryfit_video_pagination_get_page_url(1)); ?>" 
							class="video-grid__pagination-page">1</a>
						<?php
						if ($start_page > 2) {
							echo '<span class="video-grid__pagination-dots">...</span>';
						}
					}

					for ($i = $start_page; $i <= $end_page; $i++) {
						?>
						<a href="<?php echo esc_url(feryfit_video_pagination_get_page_url($i)); ?>" 
							class="video-grid__pagination-page <?php echo $current_page === $i ? 'video-grid__pagination-page--active' : ''; ?>">
							<?php echo esc_html($i); ?>
						</a>
						<?php
					}
					...
					if ($end_page < $total_pages) {
						...
						?>
						<a href="<?php echo esc_url(feryfit_video_pagination_get_page_url($total_pages)); ?>" 
							class="video-grid__pagination-page"><?php echo esc_html($total_pages); ?></a>
						<?php
					}
					?>
				</div>

				<div class="video-grid__pagination-mobile-counter">
					<span class="video-grid__pagination-current"><?php echo esc_html($current_page); ?></span>
					<span class="video-grid__pagination-separator">/</span>
					<span class="video-grid__pagination-total"><?php echo esc_html($total_pages); ?></span>
				</div>

				<?php if ($current_page == $total_pages) : ?>
					<span class="video-grid__pagination-nav video-grid__pagination-nav--next video-grid__pagination-nav--disabled">
						<svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
				...
				<?php else : ?>
					<a href="<?php echo esc_url(feryfit_video_pagination_get_page_url($current_page + 1)); ?>" 
						class="video-grid__pagination-nav video-grid__pagination-nav--next">
						<svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
